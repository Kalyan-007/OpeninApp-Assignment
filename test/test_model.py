...
def generate_waveforms(sess, net, fast_generation, global_condition):
    samples_placeholder = tf.compat.v1.placeholder(tf.compat.v1.int32)
    gc_placeholder = tf.compat.v1.placeholder(tf.compat.v1.int32) if global_condition is not None \
        else None
...
class TestNet(tf.compat.v1.test.TestCase):
    def setUp(self):
        print('TestNet setup.')
        sys.stdout.flush()
    ...
    def testEndToEndTraining(self):
        ...
        audio_placeholder = tf.compat.v1.placeholder(dtype=tf.compat.v1.float32)
        gc_placeholder = tf.compat.v1.placeholder(dtype=tf.compat.v1.int32)  \
