...
def create_variable(name, shape):
    ...
    variable = tf.compat.v1.Variable(initializer(shape=shape), name=name)
    return variable
...
def create_embedding_table(name, shape):
    if shape[0] == shape[1]:
        ...
        return tf.compat.v1.Variable(initial_val, name=name)
    else:
...
def create_bias_variable(name, shape):
    ...
    it to zero.'''
    initializer = tf.compat.v1.constant_initializer(value=0.0, dtype=tf.compat.v1.float32)
    return tf.compat.v1.Variable(initializer(shape=shape), name)
...
class WaveNetModel(object):
    ...
    def _create_dilation_layer(self, input_batch, layer_index, dilation,
        ...
        if global_condition_batch is not None:
            ...
            conv_filter = conv_filter + tf.compat.v1.nn.conv1d(global_condition_batch,
                                                     weights_gc_filter,
                                                     stride=1,
                                                     padding="SAME",
                                                     name="gc_filter")
            weights_gc_gate = variables['gc_gateweights']
            conv_gate = conv_gate + tf.compat.v1.nn.conv1d(global_condition_batch,
                                                 weights_gc_gate,
                                                 stride=1,
                                                 padding="SAME",
                                                 name="gc_gate")
        ...
        if self.use_biases:
            ...
            conv_filter = tf.compat.v1.add(conv_filter, filter_bias)
            conv_gate = tf.compat.v1.add(conv_gate, gate_bias)

        out = tf.compat.v1.tanh(conv_filter) * tf.compat.v1.sigmoid(conv_gate)

        # The 1x1 conv to produce the residual output
        weights_dense = variables['dense']
        transformed = tf.compat.v1.nn.conv1d(
            out, weights_dense, stride=1, padding="SAME", name="dense")

        # The 1x1 conv to produce the skip output
        skip_cut = tf.compat.v1.shape(out)[1] - output_width
        out_skip = tf.compat.v1.slice(out, [0, skip_cut, 0], [-1, -1, -1])
        weights_skip = variables['skip']
        skip_contribution = tf.compat.v1.nn.conv1d(
        ...
        input_cut = tf.compat.v1.shape(input_batch)[1] - tf.compat.v1.shape(transformed)[1]
        input_batch = tf.compat.v1.slice(input_batch, [0, input_cut, 0], [-1, -1, -1])
    ...
    def _generator_conv(self, input_batch, state_batch, weights):
        ...
        output = tf.compat.v1.matmul(state_batch, past_weights) + tf.compat.v1.matmul(
    ...
    def _generator_dilation_layer(self, input_batch, state_batch, layer_index,
        ...
        if global_condition_batch is not None:
            global_condition_batch = tf.compat.v1.reshape(global_condition_batch,
                                                shape=(1, -1))
            weights_gc_filter = variables['gc_filtweights']
            weights_gc_filter = weights_gc_filter[0, :, :]
            output_filter += tf.compat.v1.matmul(global_condition_batch,
                                       weights_gc_filter)
            weights_gc_gate = variables['gc_gateweights']
            weights_gc_gate = weights_gc_gate[0, :, :]
            output_gate += tf.compat.v1.matmul(global_condition_batch,
                                     weights_gc_gate)
        ...
        out = tf.compat.v1.tanh(output_filter) * tf.compat.v1.sigmoid(output_gate)

        weights_dense = variables['dense']
        transformed = tf.compat.v1.matmul(out, weights_dense[0, :, :])
        if self.use_biases:
            transformed = transformed + variables['dense_bias']

        weights_skip = variables['skip']
        skip_contribution = tf.compat.v1.matmul(out, weights_skip[0, :, :])
        if self.use_biases:
    ...
    def _create_network(self, input_batch, global_condition_batch):
        ...
        output_width = tf.compat.v1.shape(input_batch)[1] - self.receptive_field + 1
        ...
        with tf.compat.v1.name_scope('postprocessing'):
            ...
            # all up here.
            total = sum(outputs)
            transformed1 = tf.compat.v1.nn.relu(total)
            conv1 = tf.compat.v1.nn.conv1d(transformed1, w1, stride=1, padding="SAME")
            if self.use_biases:
                conv1 = tf.compat.v1.add(conv1, b1)
            transformed2 = tf.compat.v1.nn.relu(conv1)
            conv2 = tf.compat.v1.nn.conv1d(transformed2, w2, stride=1, padding="SAME")
            if self.use_biases:
                conv2 = tf.compat.v1.add(conv2, b2)
    ...
    def _create_generator(self, input_batch, global_condition_batch):
        ...
        q = tf.compat.v1.FIFOQueue(
            1,
            dtypes=tf.compat.v1.float32,
            shapes=(self.batch_size, self.quantization_channels))
        init = q.enqueue_many(
            tf.compat.v1.zeros((1, self.batch_size, self.quantization_channels)))
        ...
        with tf.compat.v1.name_scope('dilated_stack'):
            for layer_index, dilation in enumerate(self.dilations):
                with tf.compat.v1.name_scope('layer{}'.format(layer_index)):
                    ...
                    init = q.enqueue_many(
                        tf.compat.v1.zeros((dilation, self.batch_size,
                                  self.residual_channels)))
        ...
        with tf.compat.v1.name_scope('postprocessing'):
            ...
            conv1 = tf.compat.v1.matmul(transformed1, w1[0, :, :])
            if self.use_biases:
                conv1 = conv1 + b1
            transformed2 = tf.compat.v1.nn.relu(conv1)
            conv2 = tf.compat.v1.matmul(transformed2, w2[0, :, :])
            if self.use_biases:
                conv2 = conv2 + b2
    ...
    def _one_hot(self, input_batch):
        ...
        with tf.compat.v1.name_scope('one_hot_encode'):
            encoded = tf.compat.v1.one_hot(
                input_batch,
                depth=self.quantization_channels,
                dtype=tf.compat.v1.float32)
            shape = [self.batch_size, -1, self.quantization_channels]
            encoded = tf.compat.v1.reshape(encoded, shape)
        return encoded
    ...
    def _embed_gc(self, global_condition):
        ...
        if self.global_condition_cardinality is not None:
            ...
            embedding = tf.compat.v1.nn.embedding_lookup(embedding_table,
                                               global_condition)
        ...
        if embedding is not None:
            embedding = tf.compat.v1.reshape(
                embedding,
    ...
    def predict_proba(self, waveform, global_condition=None, name='wavenet'):
        ...
        with tf.compat.v1.name_scope(name):
            if self.scalar_input:
                encoded = tf.compat.v1.cast(waveform, tf.compat.v1.float32)
                encoded = tf.compat.v1.reshape(encoded, [-1, 1])
            else:
                encoded = self._one_hot(waveform)

            gc_embedding = self._embed_gc(global_condition)
            raw_output = self._create_network(encoded, gc_embedding)
            out = tf.compat.v1.reshape(raw_output, [-1, self.quantization_channels])
            # Cast to float64 to avoid bug in TensorFlow
            proba = tf.compat.v1.cast(
                tf.compat.v1.nn.softmax(tf.compat.v1.cast(out, tf.compat.v1.float64)), tf.compat.v1.float32)
            last = tf.compat.v1.slice(
                proba,
                [tf.compat.v1.shape(proba)[0] - 1, 0],
                [1, self.quantization_channels])
            return tf.compat.v1.reshape(last, [-1])
    ...
    def predict_proba_incremental(self, waveform, global_condition=None,
        ...
        with tf.compat.v1.name_scope(name):
            encoded = tf.compat.v1.one_hot(waveform, self.quantization_channels)
            encoded = tf.compat.v1.reshape(encoded, [-1, self.quantization_channels])
            gc_embedding = self._embed_gc(global_condition)
    ...
    def loss(self,
        ...
        with tf.compat.v1.name_scope(name):
            ...
            if self.scalar_input:
                network_input = tf.compat.v1.reshape(
                    tf.compat.v1.cast(input_batch, tf.compat.v1.float32),
                    [self.batch_size, -1, 1])
            else:
                network_input = encoded

            # Cut off the last sample of network input to preserve causality.
            network_input_width = tf.compat.v1.shape(network_input)[1] - 1
            network_input = tf.compat.v1.slice(network_input, [0, 0, 0],
            ...
            with tf.compat.v1.name_scope('loss'):
                ...
                target_output = tf.compat.v1.slice(
                    tf.compat.v1.reshape(
                        encoded,
                        [self.batch_size, -1, self.quantization_channels]),
                    [0, self.receptive_field, 0],
                    [-1, -1, -1])
                target_output = tf.compat.v1.reshape(target_output,
                                           [-1, self.quantization_channels])
                prediction = tf.compat.v1.reshape(raw_output,
                                        [-1, self.quantization_channels])
                loss = tf.compat.v1.nn.softmax_cross_entropy_with_logits(
                    logits=prediction,
                    labels=target_output)
                reduced_loss = tf.compat.v1.reduce_mean(loss)
                ...
                if l2_regularization_strength is None:
                    ...
                    l2_loss = tf.compat.v1.add_n([tf.compat.v1.nn.l2_loss(v)
